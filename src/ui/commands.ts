...
import { publish, PublishCancelledError } from '../publish/publisher.js'
...
export async function runPublish(plugin: InkpressPlugin) {
  ...
  const progressModal = new ProgressModal(plugin.app, () => abortController.abort())
  progressModal.open()
  ...
  try {
    ...
    if (e instanceof PublishCancelledError) {
      new Notice('Publish cancelled. Already uploaded files remain on OSS. Run Publish again to restore consistency.')
    } else {
