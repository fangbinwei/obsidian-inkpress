...
import { runPreviewCommand, runPublish } from '../ui/commands.js'
...
export class InkpressSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl).setName('AccessKey ID').addText(text =>
      text
        .setPlaceholder('LTAI...')
    ...
    new Setting(containerEl).setName('AccessKey Secret').addText(text => {
      text.inputEl.type = 'password'
      text
        .setPlaceholder('Your AccessKey Secret')
    ...
    if (
      this.plugin.settings.oss.accessKeyId &&
      detectRootAccountKey(this.plugin.settings.oss.accessKeyId)
    ) {
      const warningEl = containerEl.createDiv({ cls: 'inkpress-warning' })
      warningEl.createEl('p', {
        text: 'You may be using a root account AccessKey. We recommend creating a RAM sub-account with minimal permissions.',
      })
      warningEl.createEl('a', {
        text: 'How to create a RAM sub-account',
        href: 'https://ram.console.aliyun.com/users',
      })
    }

    new Setting(containerEl).setName('Region').addDropdown(dropdown => {
      for (const region of OSS_REGIONS)
        dropdown.addOption(region.value, region.label)
      dropdown
    ...
    new Setting(containerEl)
      ...
      .addText(text =>
        text.setValue(this.plugin.settings.oss.prefix).onChange(async value => {
        ...
        button.setButtonText('Test').onClick(async () => {
          ...
          try {
            ...
            if (result.websiteConfigured) {
              new Notice(
                'OSS connection successful! Static website hosting configured.',
              )
            } else {
              new Notice(
                `OSS connected, but couldn't set static website config: ${result.websiteError}`,
              )
            }
          } catch (e) {
            new Notice(
              `OSS connection failed: ${e instanceof Error ? e.message : String(e)}`,
            )
          } finally {
    ...
    new Setting(containerEl).addButton(button =>
      button.setButtonText('+ Add Directory').onClick(() => {
    ...
    new Setting(containerEl).setName('Upload mode').addDropdown(dropdown =>
      dropdown
    ...
    new Setting(containerEl).setName('Dead links').addDropdown(dropdown =>
      dropdown
    ...
    new Setting(containerEl)
      .setName('Preview')
      .setDesc(
        'Show what would be uploaded, deleted, and skipped — nothing is sent to OSS.',
      )
      .addButton(button =>
        button
